...
specified with that headroom. pt-MZ matters: Mozambican Portuguese uses _bairro_, _celular_,
_talão_, and local currency conventions that pt-BR and pt-PT get wrong.
...
payment state machine, and validation schemas must behave _identically_ on web and mobile because
...
- _Flutter_ — better rendering consistency and arguably better low-end performance, but the shared
...
- _Native iOS + Android_ — best performance ceiling, roughly doubles cost and timeline. Nothing in
  this scope needs it.
...
themselves phone-number-based — the buyer's MSISDN _is_ their M-Pesa/e-Mola wallet. Making phone
...
the binding constraint for a _new_ marketplace asking people to send mobile money to strangers;
...
description bodies.
**Why:** On prepaid data at Mozambican rates, a heavy app is a _cost_ to the user, not just a
...
**Decided:** Addresses carry structured fields (province → district → _bairro_) _and_ free-text
...
region adds ~120ms to _every_ request — invisible once, painful across a session, and specifically
...
mode is _money in the wrong place_. Microservices pay off when independent teams need independent
...
the cash and therefore _owes_ the platform its commission.
...
**Correction to Phase 2 docs:** the swap is _not_ "one factory function plus credentials" — the
