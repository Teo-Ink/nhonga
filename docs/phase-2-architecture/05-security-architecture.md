...
specific to *this* system called out rather than the generic ones.
...
Note what is *not* at the top: card data. There is none. Designing the system so no code path can
...
- Any admin read of KYC documents *(reading someone's identity documents is itself an auditable act)*
